  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      const generateReportContent = (data: any) => {
        ...
        // PRODUCTOS VENDIDOS (para reporte de ventas)
        if (data.productosVendidos) {
          content += `
            <div class="section">
              <div class="section-title">🛍️ PRODUCTOS VENDIDOS</div>
              <table>
                <thead>
                  <tr>
                    <th>🏷️ Producto</th>
        ...
        // ÓRDENES VENDIDAS (para reporte de ventas)
        if (data.ordenesVendidas) {
          content += `
            <div class="section">
              <div class="section-title">🧾 ÓRDENES VENDIDAS</div>
              <table>
                <thead>
                  <tr>
                    <th>ID Orden</th>
              ...
              <tr>
                ...
                <td>
                  <ul style="margin:0;padding-left:18px;">
                    ${order.items.map((item: any) => `<li>${item.name} <span style="color:#64748b">(x${item.quantity})</span></li>`).join('')}
                  </ul>
                </td>
              </tr>
            `
